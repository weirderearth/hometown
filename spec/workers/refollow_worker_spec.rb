...
describe RefollowWorker do
  ...
  describe 'perform' do
    ...
    it 'calls FollowService for local followers' do
      ...
      expect(service).to have_received(:call).with(alice, account, reblogs: true, notify: false, delivery: true, bypass_limit: true)
      expect(service).to have_received(:call).with(bob, account, reblogs: false, notify: false, delivery: true, bypass_limit: true)
    end
  end
end
